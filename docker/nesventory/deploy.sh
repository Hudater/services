#!/usr/bin/env bash
set -e
CREATE_DIRS=( "$CFG_DIR"/nesventory )
mkdir -p "${CREATE_DIRS[@]}"
# ls -alh "$BAK_CFG_DIR"/nesventory/ "$DB_CFG_DIR"
ls -alh "${CREATE_DIRS[@]}"
docker compose  --env-file "$BAK_CFG_DIR/nesventory/.env" -f "${COMPOSE_DIR}/nesventory/docker-compose.yml" up -d --force-recreate
