#!/usr/bin/env bash
set -e
CREATE_DIRS=( "${DATA_DIR}"/Media/Metube )
mkdir -p "${CREATE_DIRS[@]}"
# ls -alh "$BAK_CFG_DIR"/metube/ "$DB_CFG_DIR"
ls -alh "${CREATE_DIRS[@]}"
docker compose --env-file "$BAK_CFG_DIR/metube/.env" -f "${COMPOSE_DIR}/metube/docker-compose.yml" up -d --force-recreate
