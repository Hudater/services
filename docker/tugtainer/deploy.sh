#!/usr/bin/env bash
set -e
CREATE_DIRS=( "${BAK_CFG_DIR}"/tugtainer )
mkdir -p "${CREATE_DIRS[@]}"
# ls -alh "$BAK_CFG_DIR"/tugtainer/ "$DB_CFG_DIR"
ls -alh "${CREATE_DIRS[@]}"
docker compose  --env-file "$BAK_CFG_DIR/tugtainer/.env" -f "${COMPOSE_DIR}/tugtainer/docker-compose.yml" up -d --force-recreate
