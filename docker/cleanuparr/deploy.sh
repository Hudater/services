#!/usr/bin/env bash
set -e
CREATE_DIRS=( ${CFG_DIR}/cleanuparr )
mkdir -p "${CREATE_DIRS[@]}"
# ls -alh "$BAK_CFG_DIR"/cleanuparr/ "$DB_CFG_DIR"
ls -alh "${CREATE_DIRS[@]}"
docker compose --env-file "$BAK_CFG_DIR/cleanuparr/.env" -f "${COMPOSE_DIR}/cleanuparr/docker-compose.yml" up -d --force-recreate
