#!/usr/bin/env bash
set -e
CREATE_DIRS=( "${BAK_CFG_DIR}"/gatus "$DB_CFG_DIR"/gatus )
mkdir -p "${CREATE_DIRS[@]}"
# ls -alh "$BAK_CFG_DIR"/gatus/ "$DB_CFG_DIR"
ls -alh "${CREATE_DIRS[@]}"
docker compose -f "${COMPOSE_DIR}/gatus/docker-compose.yml" up -d --force-recreate
