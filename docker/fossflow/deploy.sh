#!/usr/bin/env bash
set -e
CREATE_DIRS=( "$BAK_CFG_DIR"/fossflow )
mkdir -p "${CREATE_DIRS[@]}"
# ls -alh "$BAK_CFG_DIR"/fossflow/ "$DB_CFG_DIR"
ls -alh "${CREATE_DIRS[@]}"
docker compose  --env-file "$BAK_CFG_DIR/fossflow/.env" -f "${COMPOSE_DIR}/fossflow/docker-compose.yml" up -d --force-recreate
